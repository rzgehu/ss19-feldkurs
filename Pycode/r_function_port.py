#!/usr/bin/env python3
"""Nicolas Gampierakis (2019), Helen Ward (2019). Python port of
Ward's iteration used to calcualte the sensible heat flux.
"""
import math
import mpmath
import time
...
def convert_rh_to_q(rh, temp_abs, pressure):
    esat = 611.2 * mpmath.exp(17.67 * (temp_abs - 273.16) / (temp_abs - 29.66))  # Stull
...
def calc_u_star(u, z_u, z0, obukhov):
    ...
    cus = k * u / (math.log(z_u / z0) - psi_m(obukhov, z=z_u) + psi_m(obukhov, z=z0))
    return cus
...
def ward_iteration(dataframe, index, zm_bls):
    ...
    while iteration_step <= 5:
        f_mo = con_T1 * (1 - con_T2 * (zm_bls / dataframe["obukhov"].loc[index])) ** (
            -2 / 3
        )
        dataframe["u_star"].loc[index] = calc_u_star(
            u=dataframe["windspeed"].loc[index],
            z_u=zm_bls,
            z0=z0,
            obukhov=dataframe["obukhov"].loc[index],
        )
        ...
        obukhov_new = (
            dataframe["temperature"].loc[index] * dataframe["u_star"].loc[index] ** 2
        ) / (g * k * dataframe["theta_star"].loc[index])
        # Store difference between new and current Obukhov length
        dataframe["obukhov_diff"].loc[index] = (
            obukhov_new - dataframe["obukhov"].loc[index]
        )
        dataframe.loc[index, "l_ob"] = obukhov_new
        # Calculate new heat flux
        h_new = (
            (-1)
            * dataframe["rho_air"].loc[index]
            * cp
            * dataframe["u_star"].loc[index]
            * dataframe["theta_star"].loc[index]
        )
